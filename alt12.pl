		...
		(   
		    ...
		    (	(   
				...
				(   
				    ...
				    error([File,M:P/A]),
				    assert(test_data(File,M:P/A,test(Predicate) :- TestGoal))
				) ;   
				(   
...
generate_tests_for_goal(Goal) :-
	...
	setof(File,test_data(File,_,_),Files),
	findall([file(File),M:P/A],(member(File,Files),test_data(File,M:P/A,_)),MPAs),
