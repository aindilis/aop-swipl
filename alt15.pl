...
:- dynamic saved_output_3/1.
:- dynamic autotestTests/1.
:- dynamic mode/0.
...
generate_tests_files(AllMPATests) :-
	...
	error_nl,
	with_output_to_predicate([X]>>(assert(saved_output_2(X))),generate_tests_file(MPATests)),
	saved_output_2(Atom),
	retractall(saved_output_2(_)),
	write_data_to_file(Atom,OutputFile),
	assert(autotestTests(File)),
	error('Wrote to file: '),
	error(OutputFile),
	error_nl,
	fail.
...
write_autotest_file :-
	error([wtf1]),
	findall(File,autotestTests(File),Files),
	error([wtf2]),
	(   with_output_to_predicate([X]>>(assert(saved_output_3(X))),write_autotestTests(Files)) -> true ; true),
	saved_output_3(Atom),
	error([atom,Atom]),error_nl,
	retractall(saved_output_3(_)),
	atomic_list_concat([
