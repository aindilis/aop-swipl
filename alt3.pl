...
should_be_wrapped(Goal) :-
	...
	(   P = ',' ->
	    ...
	    (
	     ...
	     not(my_member(P,[my_member,my_view,should_be_wrapped,do_aop_code_before,do_aop_code_after,goal_expansion,aop_advice])),
...
do_aop_code_after(Goal,Result,Output) :-
	Goal =.. [P|A],
	with_output_to(user_error,(write('> '),write_term([predicate(P),args(A),output(Output),hasResult(Result)],[quoted(true)]),nl)).
